...
class Encoder_Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.encoder_on = DenseNet(growthRate=params['growthRate'], reduction=params['reduction'], bottleneck=params['bottleneck'], use_dropout=params['use_dropout'], inchannel=3)
    ...
    def forward(self, params, x, x_mask, y, y_mask, y_reverse,y_mask_reverse,one_step=False):
        ...
        ctx_offline = self.encoder1(offline_x)
        ctx_online = self.encoder1(online_x)

        ctx_init = torch.concat((ctx_offline, ctx_online), dim = 1)

        if params['L2R'] == 1:        
            ctx_mean1 = (ctx_init * ctx_mask[:, None, :, :]).sum(3).sum(2) / ctx_mask.sum(2).sum(1)[:, None]
